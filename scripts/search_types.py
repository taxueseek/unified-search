#!/usr/bin/env python3
"""
types.py — Unified Search 统一类型系统

定义 SearchResult 数据类和 normalize_result 统一转换函数，
将所有引擎的原始输出统一为结构化格式。
...
def normalize_result(item: dict, engine: str, default_score: float = 0.5) -> SearchResult:
    ...
    处理字段名混乱问题（score/relevance_score/snippet/content/summary 混用）。
    """
    snippet = (
        item.get("snippet")
        or item.get("content")
